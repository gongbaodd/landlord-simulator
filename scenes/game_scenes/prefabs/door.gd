...
func _ready() -> void:
	if not room_scene: room_scene = load("res://scenes/game_scenes/game_scene_dev.tscn") # will cause circular dependency if used in the export
	has_key = GameManager.door_states[name]["has_key"]
